...
const Dashboard = () => {
  const [data, setData] = useState({ employees: [], attendance: [], vacations: [], complaints: [] });
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');

  useEffect(() => {
    let cancelled = false;
    Promise.all(['employees', 'attendance', 'vacations', 'complaints'].map((resource) => apiFetch(`/${resource}`).then((response) => {
      if (!response.ok) throw new Error('No se pudieron cargar las métricas.');
      return response.json();
    }))).then(([employees, attendance, vacations, complaints]) => {
      if (!cancelled) setData({ employees, attendance, vacations, complaints });
    }).catch((loadError) => {
  ...
  const presentCount = todayAttendance.filter((record) => record.status === 'Presente').length;
  const occupancyValue = todayAttendance.length ? `${Math.round((presentCount / todayAttendance.length) * 100)}%` : 'Sin datos';
  const activeReservations = data.vacations.filter((record) => ['Pendiente', 'Aprobada'].includes(record.status)).length;
  const pendingPayments = data.complaints.filter((record) => ['Pendiente', 'PENDING'].includes(record.status)).length;
  const activities = [
    ...data.attendance.map((record) => ({ date: record.date, text: `${fullName(record.employee)} tiene una reserva registrada${record.checkIn ? ` desde las ${record.checkIn}` : ''}.`, color: 'bg-green-500' })),
    ...data.vacations.map((record) => ({ date: record.requestedAt, text: `${fullName(record.employee)} reservó del ${dateText(record.startDate)} al ${dateText(record.endDate)}.`, color: 'bg-yellow-500' })),
    ...data.complaints.map((record) => ({ date: record.createdAt?.slice(0, 10), text: `Pago #${record.id}: ${record.title}.`, color: 'bg-orange-500' })),
  ...
  return (
    <div className="mx-auto w-full max-w-[1600px] space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4"><StatCard title="Huéspedes registrados" value={loading ? '...' : data.employees.length} icon={Users} color="bg-primary-600" /><StatCard title="Ocupación de hoy" value={loading ? '...' : occupancyValue} icon={BedDouble} color="bg-green-600" /><StatCard title="Reservas activas" value={loading ? '...' : activeReservations} icon={CalendarCheck} color="bg-amber-500" /><StatCard title="Pagos pendientes" value={loading ? '...' : pendingPayments} icon={CreditCard} color="bg-orange-500" /></div>
